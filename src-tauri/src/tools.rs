...
async fn save_file_to_downloads(filename: &str, content: String) -> io::Result<PathBuf> {
    ...
    let user_dirs = UserDirs::new().ok_or_else(|| io::Error::new(
        io::ErrorKind::NotFound, "Could not find user directories"))?;

    // Get the Downloads directory
    let downloads_dir = user_dirs.download_dir().ok_or_else(|| io::Error::new(
        io::ErrorKind::NotFound, "Could not find Downloads directory"))?;
