...
export default class Home extends Component {
    constructor(props) {
        ...
        this.state = {
            stock_data: {},
            // ticker_symbols: ["IBM", "AAPL", "TSLA", "AMZN", "MSFT", "INTC", "NFLX", "COST", "NVDA", "AMD", "GOOGL", "META", "ALB"]
    ...
    async componentDidMount() {
        ...
        const fetchStockData = async (ticker_symbol) => {
            try {
                const company_name = await invoke("get_company_name", { tickerSymbol: ticker_symbol });
                const company_exchange = await invoke("get_company_exchange", { tickerSymbol: ticker_symbol });
                const ticker_data = await request_ticker_data(ticker_symbol);
