...
export default class Playground extends Component {
  ...
  constructor(props) {
    ...
    this.sorting_content = {
      MarketCap: () => {
        this.set_sorting("MarketCap");
      },
      Volitility: () => {
        this.set_sorting("Volitility");
      },
      Bullish: () => {
        this.set_sorting("Bullish");
      },
      Bearish: () => {
        this.set_sorting("Bearish");
      },
    };
  }
  ...
  async searching_ticker(ticker_search) {
    ...
    const sorted_tickers = all_tickers.filter(
      (ticker) =>
        !clean_ticker(ticker_search).includes(".") &&
        ticker
          .toLocaleUpperCase()
          .startsWith(clean_ticker(ticker_search).toLocaleUpperCase()),
    );
    this.setState({
      ticker_symbols: sorted_tickers.slice(0, SHOWN_TICKER_AMOUNT),
    });
  ...
  async set_tickers(ticker_symbols, func = () => {}) {
    if (!func)
      return this.setState(
        { ticker_symbols, ticker_symbols_before_sort: ticker_symbols },
        () => this.set_sorting(this.state.sort_method),
      );
    this.setState(
      { ticker_symbols, ticker_symbols_before_sort: ticker_symbols },
      func,
  ...
  async set_sector(sector) {
    ...
    const tickers_in_sector = top_500.filter(
      (ticker_symbol) => all_nasdaq_info[ticker_symbol].sector === sector,
    );
  ...
  async set_sorting(sort_method) {
    ...
    switch (sort_method) {
      case "MarketCap": {
        ...
        const sorted_by_weight = weights
      ...
      case "Volitility": {
        const change_promises = ticker_symbols.map(async (ticker_symbol) => {
          const change = unformat_number(
            all_nasdaq_info[ticker_symbol].pctchange,
          );
          return { ticker_symbol, change };
        });
        const changes = await Promise.all(change_promises);
        // sort by highest weight
        const sorted_by_weight = changes
  ...
  async componentDidMount() {
    ...
    for (const sector of Object.keys(custom_sectors)) {
      if (custom_sectors[sector].function) {
        custom_sectors[sector] = {
          ...custom_sectors[sector],
          ...(await eval(custom_sectors[sector].function)),
        };
      }
    ...
    this.setState({ custom_sectors, default_sector: default_sector }, () =>
      this.set_sector(TOP_20),
    );
  }
  ...
  render() {
    ...
    const widgetSize =
      settings.Playground_Page.settings.default_widget_size.value || "medium";
    return (
      <ThemeProvider theme={theme}>
        <div className={"playground"}>
          <div className={"generic-header"}>
            ...
            >
              <SectorSelect
                set_sector={this.set_sector}
                custom_sectors={Object.keys(custom_sectors)}
                default_sector={default_sector}
              />
              <EasySelection
                label="Sort"
                content={this.sorting_content}
                default={sort_method}
              />
              <Link
                to="/home"
                className={"homepage-navButton"}
                style={{ marginLeft: "auto", order: 2, height: "auto" }}
              >
                Home
              </Link>
              <LoadingTextField
                ...
                variant="outlined"
                color="primary"
                onChange={(e) => this.searching_ticker(e.target.value)}
                value={ticker_search}
                loading={show_loading_ticker_search}
              />
              <Button
                onClick={() => {
                  if (show_sort_button) this.set_sorting(sort_method);
                  else
                    this.setState({
                      ticker_symbols: this.state.ticker_symbols_before_sort,
                    });
                  this.setState({ show_sort_button: !show_sort_button });
                }}
              >
                {show_sort_button ? "Sort" : "Unsort"}
              </Button>
              {ticker_search !== "" && (
                <Button onClick={() => this.searching_ticker("")}>Clear</Button>
              )}
            </SoftPaper>
          </div>
